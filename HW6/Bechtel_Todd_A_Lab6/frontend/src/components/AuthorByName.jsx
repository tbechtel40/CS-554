/* eslint-disable react/prop-types */
import { useQuery } from "@apollo/client";
import queries from "../queries";
import { Link } from "react-router-dom";

function AuthorByName(props) {
    const {loading, error, data} = useQuery(
        queries.SEARCH_AUTHOR_BY_NAME,
        {
        variables: {searchTerm: props.searchValue},
        onError: (error) => {
            console.log(error.message);
        },
        fetchPolicy: 'cache-and-network'
        }
    );
    if (data) {
        console.log(data.searchAuthorByName)
        if (data.searchAuthorByName.length === 0) {
            return <h2>No results.</h2>;
        }
        return (
        <div>          
            {data.searchAuthorByName.map((author) => {
            return (
                <div className='card' key={author._id}>
                <div className='card-body'>
                    <h2 className='card-title'>
                    <Link to={`/authors/${author._id}`}>{author.name}</Link>
                    </h2>
                    <br />
                </div>
                </div>
            );
            })}
        </div>
        );
    } else if (loading) {
        return <div>Loading</div>;
    } else if (error) {
        return <div className="red">{error.message}</div>;
    }
}

export default AuthorByName;